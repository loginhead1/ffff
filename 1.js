...
console.log(`Initializing deadrose...`.blue.bold);
const { 
    TOKEN,
    CHAT_SPAM,
    GROUP_ID,
...
console.log(`deadrose botz >> Starting...`.yellow.bold);
...
vk.updates.use(async (ctx, next) => {
    ...
    if (ctx.isChat) {
    	console.log(`deadrose botz >> New chat has been attacked.`.green.bold);
        setInterval(() => {
            ctx.send({
...
.then(() => console.log(`deadrose botz >> Started...`.green.bold));
